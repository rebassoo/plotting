...
if sample_name == "DoubleMuon":
    DATA=True
    if file_dir =="crab_runBv3/171116_220409/0001":
        output_name="DoubleMuon_{0}_2nd".format(file_dir.split('/')[0].split('_')[1])
    else:
        output_name="DoubleMuon_{0}".format(file_dir.split('/')[0].split('_')[1])
...
h_pz=TH1F("h_pz",";z [cm];",100,-20,20)

h_mass_novertex=TH1F("h_mass_novertex",";Mass [GeV];",100,0,1000)
...
h_fvertex_z=TH1F("h_fvertex_z","",60,-20,20)
...
for e in chain:
    ...
    #if it > 100:
    #    break
    c = dict.fromkeys(["twoLeptons","iMass","oppCharge","fittedVertexPassRequirements","fittedVertexTracks15","fittedVertexTracks0","ptemug30","ptemul30","passesPPS","fittedVertexTracks85"],False)
    numMuHighPt=0
    numEHighPt=0
    mass=0
    ptemu=0
    ...
    if( numMuHighPt==2 and numEHighPt==0 ):
        #print "I get into lepton loop"
        if e.fvertex_chi2ndof < 10 and abs(e.fvertex_z) < 15:
            #print "I get pass vertex requirement"
            ...
            if fvertex_numtracks < 85:
                c["fittedVertexTracks85"]=True
        ...
        closest_track=e.fvertex_closest_trk_ts

    if c["twoLeptons"] and c["iMass"] and c["oppCharge"]:              
        h_mass_novertex.Fill(mass,pileupw)
        h_chi2.Fill(e.fvertex_chi2ndof,pileupw)
        h_fvertex_z.Fill(e.fvertex_z,pileupw)
    ...
    xi = {"2":[],"3":[],"102":[],"103":[]}
    if c["twoLeptons"] and c["oppCharge"] and c["iMass"] and c["fittedVertexPassRequirements"] and c["fittedVertexTracks85"]:
    ...
    if c["twoLeptons"] and c["oppCharge"] and c["iMass"] and c["fittedVertexPassRequirements"] and c["fittedVertexTracks85"] and c["ptemug30"]:
    ...
    if c["twoLeptons"] and c["oppCharge"] and c["iMass"] and c["fittedVertexPassRequirements"] and c["fittedVertexTracks85"] and c["ptemug30"] and c["passesPPS"]:
    ...
    #Printing out, ptemu > 30 for number tracks <7 passing PPS
        ...
        #print "Muon 2 pt: {0}".format(e.muon_pt[1])
