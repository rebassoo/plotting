...
#All these will have 3 distributions, consider removing because I am using 5+
...
for e in chain:
    ...
    if mjet_veto and passesBoosted and tau21<0.6:
        ...
        if sample=="ExclusiveMC":
            passesPPS=passPPSGeneralSignal(e,xi,signal_bin,sample)
        if sample=="Data":
            passesPPS=passPPSGeneralData(e,xi,signal_bin)
        ...
        if sample=="MC":
            passesPPS=passPPSGeneralMixDataMC(e,xi,signal_bin,sample,era)
    #Mixing in protons with data events not passing PPS, currently there is overlap between control regions.
    passPPS2=False
    #if not passesPPSmultiRP or not passesPPSmultiPixel or not passesPPSPixelPixel:
    #    passesPPS=False
    if mjet_veto and passesBoosted and jet_pruning and DATA and not passesPPS:
        ...
        #print "passPPS2 inital: ",passPPS2
        passPPS2=passPPSGeneralMixDataMC(e,xi,signal_bin,sample,era)
        #while not passPPS2:
        #    passPPS2=passPPSGeneralMixDataMC(e,xi,signal_bin,sample,era)
        #    if not passPPS2:
        #        del xi["23"][:]
    ...
    #Maybe this should go before xi veto of 0.04?
    if not ExclusiveMC and not DATA and mjet_veto and passesBoosted and jet_pruning and pfcand_nextracks<5:
        for i in range(1,100):
            ...
            passPPS_100=passPPSGeneralMixDataMC(e,xi_signal,signal_bin,sample,era)
            if not passPPS_100: continue
    ...
    if mjet_veto and passesBoosted and jet_pruning:
        if len(xi["23"]) == 2 and len(xi["123"])==2:
            xi_mult_45=max(xi["23"])
            xi_mult_56=max(xi["123"])
    ...
    if mjet_veto and passesBoosted and jet_pruning and passesPPS and not DATA:    
        if pfcand_nextracks<5:
            ...
            if ExclusiveMC:
                h_MWW_MX_0_4_tracks.Fill(recoMWW/M_RP,pileupw*rw_extrk*rw_passPPS*xi["weight"][0])
            else:h_MWW_MX_0_4_tracks.Fill(recoMWW/M_RP,pileupw*rw_extrk*rw_passPPS)
            ...
            if passYcut:
                if ExclusiveMC:
                    h_MWW_MX_0_4_tracks_Ycut.Fill(recoMWW/M_RP,pileupw*xi["weight"][0])
                else:h_MWW_MX_0_4_tracks_Ycut.Fill(recoMWW/M_RP,pileupw)

    if mjet_veto and passesBoosted and jet_pruning and not passesPPS and passPPS2 and DATA:    
        if pfcand_nextracks<5:
    ...
    if mjet_veto and passesBoosted and jet_pruning and not passesPPS and passPPS2 and pfcand_nextracks<16 and pfcand_nextracks>4 and DATA:
    #if mjet_veto and passesBoosted and jet_pruning and not passesPPS and passPPS2 and pfcand_nextracks<16 and pfcand_nextracks>4:
